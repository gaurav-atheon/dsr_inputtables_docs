...
select     {{ dbt_utils.surrogate_key(['all_fact_data.origin_organisation_number', 'all_fact_data.business_organisation_number',
            ...
            all_fact_data.location_function,all_fact_data.loaded_timestamp,all_fact_data.is_ghost,all_fact_data.runstartedtime
from
(
select
    src.organisation_id,
    coalesce(to_char(inv.{{location_col_name}}) , '') as organisation_location_id,
...
on inv.source_db_id = src.business_organisation_number
    group by organisation_id,{{location_col_name}},location_function,loaded_timestamp,business_organisation_number,origin_organisation_number,runstartedtime
    ) all_fact_data
