...
select
   ...
   {{ dbt_utils.surrogate_key(['base_forecast_date','forecast_date','source_db_id','subject_origin_organisation_number','subject_organisation_location_id_to',
                               'subject_business_organisation_number_from','subject_business_organisation_number_to','subject_organisation_sku','model_version']) }} as unique_key,
 ...
 {% if target.name == 'ci' %}
    from {{ ref ('stg_pln_mvt_orgdepotdaysku_ci' )}}
 {% else %}
